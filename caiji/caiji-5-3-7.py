...
class Player:
    def __init__(self, isFirst, array):
        ...
        self.array = array
        self.tree = _GameTree(isFirst, 6, 2)
    ...
    def output(self, currentRound, board, mode):
        ...
        if mode == "position" or mode == "direction":
            decision, value = self.tree.decide(
                self.tree.root, self.tree.get_depth(), -float('inf'), float('inf'), mode, currentRound)
        else:
            pass
        if value!=0 and self.tree.complex and mode=='direction':
            file=open(self.filename+".csv","a")
            writer=csv.writer(file)
            self.tree.root.shift_para(value,self.isFirst)
            writer.writerow([_Node.P1,_Node.P2,_Node.P3,_Node.P4])
            file.close()
...
class _Node:
    ...
    """
    P1=1
    P2=random.randint(0,100)/50
    P3=random.randint(0,100)/50
    P4=random.randint(0,100)/50
    eta=1e-4
    ...
    def point(self,isFirst):
        part1=self.point1(isFirst)
        part2=self.point2(isFirst)
        part3=self.point3(isFirst)
        part4=self.point4(isFirst)
        return (_Node.P1*part1 + _Node.P2*part2 + _Node.P3*part3 + _Node.P4*part4) * (2 * isFirst - 1)
    ...
    def shift_para(self,value,isFirst):
        delta=value-self.point(isFirst)
        dP2 = _Node.eta*delta*self.point2(isFirst)
        dP3 = _Node.eta*delta*self.point3(isFirst)
        dP4 = _Node.eta*delta*self.point4(isFirst)
        _Node.P2+=dP2
        _Node.P3+=dP3
        _Node.P4+=dP4
